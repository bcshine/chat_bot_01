document.addEventListener('DOMContentLoaded', () => {
    ...
    // If we have a saved API key, hide the input
    if (OPENAI_API_KEY) {
    ...
    // Toggle API key input visibility based on checkbox
    useAICheckbox.addEventListener('change', () => {
        if (useAICheckbox.checked && !OPENAI_API_KEY) {
    ...
    // Get employee response using OpenAI
    async function getEmployeeResponseFromOpenAI(userMessage) {
        try {
            ...
            conversationHistory.forEach(msg => {
                const role = msg.sender === 'user' ? '사장' : '직원';
                formattedConversation += `${role}: ${msg.message}\n`;
            });
            ...
            alert(`OpenAI API 오류: ${error.message}. 미리 준비된 응답을 사용합니다.`);
    ...
    async function handleUserMessage() {
        ...
        try {
            ...
            // Use OpenAI or fallback based on checkbox
            if (useAICheckbox.checked && OPENAI_API_KEY) {
                employeeResponse = await getEmployeeResponseFromOpenAI(message);
            } else {
